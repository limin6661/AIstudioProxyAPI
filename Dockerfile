...
# 可选：构建时代理，例如：
# ARG PROXY_ADDR="http://1.2.3.4:7890"
# Linux 下使用 host.docker.internal 可能会有问题，建议使用实际的代理地址
...
# 步骤 1: 安装系统依赖（含 Playwright 运行所需依赖）
RUN \
    ...
    libatk1.0-0 libatk-bridge2.0-0 libcups2 libdbus-1-3 libdrm2 libgbm1 libgtk-3-0 libnspr4 libnss3 \
    libx11-6 libx11-xcb1 libxcb1 libxcomposite1 libxdamage1 libxext6 libxfixes3 libxrandr2 libxrender1 libxtst6 \
    ca-certificates fonts-liberation libasound2 libpangocairo-1.0-0 libpango-1.0-0 libu2f-udev \
    supervisor curl \
    && apt-get clean && rm -rf /var/lib/apt/lists/* && \
    if [ -n "$PROXY_ADDR" ]; then rm -f /etc/apt/apt.conf.d/99proxy; fi
...
# 步骤 2: 复制 Python 包和可执行文件（先复制依赖，后复制代码）
...
# 步骤 3: 安装 camoufox + playwright firefox
RUN camoufox fetch && \
...
# 更新 browserforge 数据（脚本存在才执行，避免因为路径差异导致构建失败）
RUN if [ -f "/app/scripts/update_browserforge_data.py" ]; then \
      python /app/scripts/update_browserforge_data.py; \
    else \
      echo "skip: /app/scripts/update_browserforge_data.py not found"; \
    fi
